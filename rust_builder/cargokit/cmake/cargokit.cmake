...
function(apply_cargokit target manifest_dir lib_name any_symbol_name)
    ...
    # Windows: default path is under build/windows/x64/plugins/.../cargokit_build and
    # aws-lc-sys + CMake TryCompile nests deep enough to exceed MAX_PATH (~260).
    # Use a short per-crate directory under %TEMP% for Cargo's target dir instead.
    if(WIN32)
        file(TO_CMAKE_PATH "$ENV{TEMP}" _CARGOKIT_WIN_TEMP)
        if(NOT _CARGOKIT_WIN_TEMP)
            set(CARGOKIT_TEMP_DIR "${CMAKE_CURRENT_BINARY_DIR}/cargokit_build")
        else()
            string(SHA256 _CARGOKIT_SRC_HASH "${CMAKE_CURRENT_SOURCE_DIR}")
            string(SUBSTRING "${_CARGOKIT_SRC_HASH}" 0 8 _CARGOKIT_SRC_HASH_SHORT)
            set(CARGOKIT_TEMP_DIR "${_CARGOKIT_WIN_TEMP}/dd_cargokit_${_CARGOKIT_SRC_HASH_SHORT}")
        endif()
    else()
        set(CARGOKIT_TEMP_DIR "${CMAKE_CURRENT_BINARY_DIR}/cargokit_build")
    endif()
