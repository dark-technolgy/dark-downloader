...
class ToolkitNotifier extends Notifier<ToolkitState> {
  ...
  Future<void> convertToMp3() async {
    ...
    state = state.copyWith(
      status: ToolkitTaskStatus.processing,
      statusMessage: 'جاري استخراج الصوت بجودة 320kbps...',
    );
    ...
    try {
      ...
      state = state.copyWith(
        status: ToolkitTaskStatus.success,
        statusMessage: 'تم الحفظ في: $outputPath',
      );
    } catch (e) {
      state = state.copyWith(
        status: ToolkitTaskStatus.error,
        statusMessage: 'فشل التحويل: $e',
      );
    }
  }
  ...
  Future<void> compressVideo() async {
    ...
    state = state.copyWith(
      status: ToolkitTaskStatus.processing,
      statusMessage: 'جاري ضغط الفيديو مع الحفاظ على الجودة...',
    );
    ...
    final ffmpegPath = await resolveDesktopFfmpegPath();
    try {
      await MediaProcessor.compressVideo(
        inputPath: inputPath,
        outputPath: outputPath,
        ffmpegPath: ffmpegPath,
      );
      state = state.copyWith(
        status: ToolkitTaskStatus.success,
        statusMessage: 'تم الضغط بنجاح: $outputPath',
      );
    } catch (e) {
      state = state.copyWith(
        status: ToolkitTaskStatus.error,
        statusMessage: 'فشل الضغط: $e',
      );
    }
  }
