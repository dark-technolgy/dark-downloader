...
async fn main() {
    ...
    println!("Consume 1: {}", client.get(&final_url).send().await.unwrap().status());

    // 3. Consume it again (Simulating the actual download)
    println!("Consume 2: {}", client.get(&final_url).send().await.unwrap().status());
    
    // 4. Consume it third time
    println!("Consume 3: {}", client.get(&final_url).send().await.unwrap().status());
}
