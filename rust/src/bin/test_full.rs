...
async fn main() {
    ...
    let r = client1.get(&full_url).header(header::RANGE, "bytes=0-0").send().await.unwrap();
    println!("Probe Status: {}", r.status());
    
    let chunk_req = client1.get(&full_url).header(header::RANGE, "bytes=0-1000").send().await.unwrap();
    println!("Chunk Status: {}", chunk_req.status());
    if !chunk_req.status().is_success() && chunk_req.status() != reqwest::StatusCode::PARTIAL_CONTENT {
