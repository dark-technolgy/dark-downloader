    ...
    match extract(yt_url).await {
        Ok(ExtractionResult::Video(video)) => {
            ...
            for s in video.streams.iter().filter(|s| s.has_video) {
                println!("- {} ({} fps) [Audio: {}]", s.quality, s.fps.unwrap_or(0.0), s.has_audio);
            }
            
            // Find 1440p or 2160p video (or best available)
            let mut v_stream = video.streams.iter().filter(|s| s.has_video).collect::<Vec<_>>();
            v_stream.sort_by(|a, b| b.quality.cmp(&a.quality)); // descending
            let best_video = v_stream.first().map(|s| s.url.clone());
            
            // Find best audio
            let mut a_stream = video.streams.iter().filter(|s| s.has_audio && !s.has_video).collect::<Vec<_>>();
            ...
            if let Some(v_url) = best_video {
                ...
                let monitor_yt = tokio::spawn(async move {
                    loop {
                        if let Some(progress) = get_job_progress(job_id_clone_yt.clone()) {
                            println!("Progress: {:.2}% ({} / {}) - Speed: {} B/s", 
                                progress.percent, progress.downloaded_bytes, progress.total_bytes, progress.speed_bytes_sec);
                ...
                println!("Starting YouTube Download with audio: {}", best_audio.is_some());
