...
pub async fn extract_ultra(url: String) -> Result<VideoInfoResult> {
    debug_log::log_debug(&format!(
        "Dark Engine: Initiating Ultra-Extraction for {}",
        url
    ));
    ...
    let download_re =
        regex::Regex::new(r#"href=["'](?:https?://[^/]+)?(/download/[0-9]+/[0-9]+/[^/"']+/?)"#)
            .ok();
    ...
    if streams.is_empty() {
        debug_log::log_debug(
            "Dark Engine: Static analysis failed, attempting Headless JS Execution...",
        );
    ...
    for mut s in streams {
        if !seen.contains(&s.url) {
            ...
            if s.quality.contains("Auto") {
                if s.url.contains("1080") {
                    s.quality = "1080p (FHD)".into();
                } else if s.url.contains("720") {
                    s.quality = "720p (HD)".into();
                }
            }
    ...
    if final_streams.is_empty() {
        return Err(anyhow!(
            "Engine reached its limit. Site may be encrypted or restricted."
        ));
    }
